...
# ╔═╡ 7e5b3522-a604-11ed-1661-a57692f7a513
begin
...
# ╔═╡ 2efc1a33-bd67-4eb0-8c5e-d6fd378ec5a4
ds = Parquet2.Dataset("C:\\Users\\Hemant\\Desktop\\jl_Files\\MIT_18.S097_Introduction-to-Julia-for-Data-Science\\Day-1b_Working-with-tabular-data\\puzzles.parquet")

# ╔═╡ d37c0ca8-512c-452b-b8b7-a9e051081099
df = DataFrame(ds)

# ╔═╡ a7e6449b-dc0b-4940-8b01-7068eeb7decb
describe(df)

# ╔═╡ 771c938d-53a2-4ed0-8d1f-3f9c7242a107
histogram(df.Rating, legend=false, title="Rating")

# ╔═╡ 1e124076-2f48-4f95-81da-82fd8290bd76
plot(
     histogram(df.Rating, legend=false, title="Rating"),
     histogram(df.Popularity, legend=false, title="Popularity"),
     histogram(log1p.(df.NbPlays), legend=false, title="Number of plays"),
     layout=(3, 1), size=(600, 750)
) #log. , log1p.

# ╔═╡ ea72d611-faa0-41f5-9ded-fbce0c65a9c4
df.Themes

# ╔═╡ 40822d51-ff8d-43f4-847c-90ed8180637a
split(first(df.Themes))

# ╔═╡ e57badaf-126a-4446-81f1-e23cd1316227
themes = String[]

# ╔═╡ 59f1319b-fc5b-4c6c-b0d4-2cee89a40298
for ts in df.Themes
    for t in split(ts)
        push!(themes, t)
    end
...
# ╔═╡ 1b2b9746-e8f8-4801-a413-84943a89b2c9
sort(unique(themes))

# ╔═╡ 1d465575-c7ff-4298-8d98-4b2446960790
freqtable(themes)

# ╔═╡ 207b2577-212d-4689-b1ed-97319b52ec06
sort(unique(t for t in themes if contains(lowercase(t), "mate")))

# ╔═╡ 7617e2c5-dd52-4b69-9bd5-19909d56b14a
for i in 1:5
	df[:, "mateIn$i"] = contains.(df.Themes, "mateIn$i")
end

# ╔═╡ fcd2ff63-40d4-46df-bc7b-25d5613d3c17
df

# ╔═╡ aca43e5f-a96d-4553-9159-c51e5a496a4d
select!(df, Not([:Moves, :Themes]))

# ╔═╡ f4c66c74-ce44-4ef1-a0e7-7c84498a7167
model = lm(@formula(Popularity ~ mateIn1 + mateIn2 + mateIn3 + mateIn4 + mateIn5), df)

# ╔═╡ 2f2f8e40-a182-4245-9285-0a555755449b
DataFrame(coeftable(model))

# ╔═╡ 7bca8ce9-2f42-4fdf-a255-cdd62e197ee5
gdf = groupby(df, :OpeningFamily, sort=true)

# ╔═╡ 83aab19d-334e-45d9-b227-42613b60c6a2
@combine(gdf, :Popularity_mean = mean(:Popularity))

# ╔═╡ d3deaeec-bfe7-4014-b1b2-8c54d59e11eb
@chain gdf begin
	@combine(:Popularity = mean(:Popularity))
	sort(:Popularity, rev=true)
end

# ╔═╡ 55d3ec52-fedb-4116-b44d-6cc157a25e77
...
# ╔═╡ 541094dc-bc4d-47bc-8917-e2be493da76b
hline!([total_mean_popularity], label = "global mean", width=3)

# ╔═╡ f5a2ce78-c971-404c-a8d6-b5c4d9ae10b8
...
# ╔═╡ 5a530e53-9500-4e7d-828a-71186abab876
sort(agg_popularity_frequent, :zscore, rev=true, by=abs)

# ╔═╡ e84c0ad9-a77f-4520-9d5e-88a8f1af8fd9
...
# ╔═╡ defdaac6-662d-4f69-b82c-14ad59150904
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "eb9ce11b7516dab097072b719da5cbfae2c7e9d7"
...
git-tree-sha1 = "66369db4570bcd852bde2dd39beaa559bc9890dd"
...
git-tree-sha1 = "22cf8c4315c417300b753c0def0fcb94e83d79b1"
...
[[deps.Parquet2]]
deps = ["AbstractTrees", "BitIntegers", "CodecLz4", "CodecZlib", "CodecZstd", "DataAPI", "Dates", "DecFP", "FilePathsBase", "FillArrays", "JSON3", "LazyArrays", "LightBSON", "Mmap", "OrderedCollections", "PooledArrays", "SentinelArrays", "Snappy", "StaticArrays", "TableOperations", "Tables", "Thrift", "Transducers", "UUIDs", "WeakRefStrings"]
git-tree-sha1 = "e82cfec689da90ba8bb46e9ed5878b80459480d3"
